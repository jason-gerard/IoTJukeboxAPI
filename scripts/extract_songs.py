...
files = glob.glob(f'../song_data/**/*.ino', recursive=True)
for file in files:
    print(file)
    with open(file, 'r') as reader:
        ...
        for line in reader:
            if line.strip() == "" or line.strip().startswith("//") or line.strip().startswith("/*") or line.strip().startswith("*") or line.strip().startswith("*/"):
                ...
                parts = [part.strip() for part in parts if part.strip() != "" and "//" not in part]
                for part in parts:
                    melody.append(part)
    ...
    with open(f"../songs/{file_name}", "w") as writer:
        writer.write(tempo)
        writer.write("\n")
        writer.write(",".join(melody))
